...
SERIAL_EXAMPLE::SERIAL_EXAMPLE() : port(io), timeout(io) {
...
void SERIAL_EXAMPLE::open_port( string port_name, int baudRate ) {
	...
	serial_thread = boost::thread(boost::bind(&SERIAL_EXAMPLE::serialLoop, this));
}
...
void SERIAL_EXAMPLE::writeData( unsigned char *ptr, int length ) {
...
void SERIAL_EXAMPLE::read_handler(bool& data_available, deadline_timer& timeout, 
...
void SERIAL_EXAMPLE::wait_callback(serial_port& ser_port, const boost::system::error_code& error)
{
	if (error) {
...
void SERIAL_EXAMPLE::processWriteBuffer() {
	// take out lock
	writeBufferMutex.lock();
	if ( bytesToWrite > 0 ) {
...
void SERIAL_EXAMPLE::serialLoop() {
	while (1) {
		...
		port.async_read_some( buffer( &readBuffer[0], 100 ), 
				boost::bind( &SERIAL_EXAMPLE::read_handler, this, boost::ref(data_available), boost::ref(timeout), 
		...
		timeout.async_wait( boost::bind( &SERIAL_EXAMPLE::wait_callback, this, boost::ref(port), 
				boost::asio::placeholders::error ) );
