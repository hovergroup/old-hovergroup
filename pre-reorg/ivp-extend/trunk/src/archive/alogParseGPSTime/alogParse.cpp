...
int num_clock_samples = 0;
double clock_offset_sum = 0;
...
int main (	int argc, char *argv[] ) {
	...
	while( entry.getStatus() != "eof" ) {
		...
		if ( entry.getStatus() == "okay" ) {
			...
			if ( key == "GPS_TIME_SECONDS" ) {
			    num_clock_samples++;
			    double val = atof(entry.getStringVal().c_str());
			    clock_offset_sum += (val - entry.getTimeStamp());
			}
	...
	std::vector<ALogEntry> gpsed_entries;
	// adjust time for gps offset
	double clock_offset = clock_offset_sum / ( (double) num_clock_samples );
	cout << "Offset: " << clock_offset << endl;
	for ( int i=0; i<entries.size(); i++ ) {
	    ALogEntry gpsed_entry;
	    if ( entries[i].isNumerical() ) {
	        gpsed_entry.set(
                    entries[i].getTimeStamp() + clock_offset,
                    entries[i].getVarName(),
                    entries[i].getSource(),
                    entries[i].getSrcAux(),
                    entries[i].getDoubleVal() );
	    } else {
	        gpsed_entry.set(
                    entries[i].getTimeStamp() + clock_offset,
                    entries[i].getVarName(),
	    ...
	    gpsed_entries.push_back(gpsed_entry);
	}
	...
	if ( use_sync_variable && restrict_to_backsearch ) {
		...
		map<string,string> current_value;
		map<string,double> current_times;
		double last_post_time = gpsed_entries[0].getTimeStamp();
		for ( int i=0; i<variables.size(); i++ ) {
			current_value[variables[i]] = "-1";
			current_times[variables[i]] = -100000000;
		}
