...
% gamma = deg2rad(0.1*gamma);% + deg2rad(15);
...
alpha = deg2rad([10, 20, 30, 40, 30]);
gamma = deg2rad([10, 20, 30, 40, 30]);
beta = deg2rad([10, 20, 30, 40, 30]);
...
while(i < N)
  ...
  if 0 < s.BytesAvailable
    ...
    %t = [t, (toc-t0)];
    for j=1:5
        position(j,i) = data(offset_position + (j-1)*width,i) + 256*data(offset_position + (j-1)*width + 1, i);        
    end
    ...
    disp(' ');
    disp(['      at: alpha=', num2str(alpha_m),' gamma=', num2str(gamma_m),' beta=', num2str(beta_m)]);
    disp(['going to: alpha=', num2str(rad2deg(alpha(k))), ' gamma=', num2str(rad2deg(gamma(k))), ' beta=', num2str(rad2deg(beta(k))), ' ']);
    ...
    if (min(demand_type == zeros(1,5)) == 1 )
       ...
       disp(['alpha=', num2str(rad2deg(alpha(k))), ' gamma=', num2str(rad2deg(gamma(k))), ' beta=', num2str(rad2deg(beta(k))), ' ']);
       disp('-----------------------------------------');
       %pause(1)
...
if (~isempty(data()))
    ...
    for i=1:5
      for j =1:M
        position(i,j) = data(offset_position + (i-1)*width,j) + 256*data(offset_position + (i-1)*width + 1, j);
        current(i,j) = data(offset_current + (i-1)*width,j) + 256*data(offset_current + (i-1)*width + 1, j);
