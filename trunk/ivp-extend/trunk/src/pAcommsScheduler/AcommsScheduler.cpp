...
bool AcommsScheduler::OnNewMail(MOOSMSG_LIST &NewMail) {
    ...
    for (p = NewMail.begin(); p != NewMail.end(); p++) {
        ...
        if (key == "ACOMMS_DRIVER_STATUS") {
            HoverAcomms::DriverStatus val =
                    static_cast<HoverAcomms::DriverStatus>(msg.GetDouble());
            if (val == HoverAcomms::RECEIVING) {
                ...
                m_dataComplete = false;
                m_receiveComplete = false;
            } else if (m_driverStatus == HoverAcomms::RECEIVING
                    && val == HoverAcomms::READY) {
                // end of receipt
    ...
    if (m_dataComplete && m_receiveComplete) { // got both status timings and data output
        m_observations.push_back(
                std::pair<double, double>(m_lastStart, m_lastEnd));
...
bool AcommsScheduler::Iterate() {
    ...
    double m_offset = modf(m_time / m_period, &m_slot) * m_period
            - m_meanOffset;
    if (m_offset < 0)
        m_offset += m_period;
    ...
    if (0 < m_offset && m_offset <= m_meanDuration) {
        updateState(lock);
    } else if (m_meanDuration < m_offset
            && m_offset <= m_meanDuration + m_postLock) {
    ...
    if (m_haveTransmission && m_state == unlocked
            && m_driverStatus == HoverAcomms::READY) {
        ...
        case ACOMMS_TRANSMIT_DATA_BINARY:
            m_Comms.Notify("SCHEDULER_TRANSMIT_DATA_BINARY",
                    (void*) m_transmissionData.data(),
                    m_transmissionData.size());
            break;

        case ACOMMS_TRANSMIT:
            m_Comms.Notify("SCHEDULER_TRANSMIT",
                    (void*) m_transmissionData.data(),
                    m_transmissionData.size());
            break;
