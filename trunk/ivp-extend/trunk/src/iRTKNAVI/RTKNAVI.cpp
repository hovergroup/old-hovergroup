...
bool RTKNAVI::OnStartUp() {
    ...
    if (!ok1 || !ok2) {
        std::cout << "Error reading Lat/Long origin from MOOS file." << std::endl;
        return false;
    }
...
void RTKNAVI::ioLoop() {
	while (true) {
		...
		sock.async_read_some( buffer( &readBuffer[0], 1000 ),
				boost::bind( &RTKNAVI::read_handler, this, boost::ref(data_available), boost::ref(timeout),
		...
		timeout.async_wait( boost::bind( &RTKNAVI::wait_callback, this, boost::ref(sock),
				boost::asio::placeholders::error ) );
		...
		if (data_available) {
			parseLine(std::string(readBuffer.begin(), readBuffer.begin()+=asyncBytesRead));
		}
	}
}
...
void RTKNAVI::wait_callback(tcp::socket& ser_port, const boost::system::error_code& error)
{
	if (error) {
