...
void icarus_driver::open_port( string port_name, int baudRate ) {
	...
	serial_thread = boost::thread(boost::bind(&icarus_driver::serialLoop, this));
}
...
void icarus_driver::wait_callback(serial_port& ser_port, const boost::system::error_code& error)
{
	if (error) {
...
void icarus_driver::serialLoop() {
	while (!stop_requested) {
		...
		port.async_read_some( buffer( &readBuffer[buffer_index], 1000-buffer_index ),
				boost::bind( &icarus_driver::read_handler, this, boost::ref(data_available), boost::ref(timeout),
		...
		timeout.async_wait( boost::bind( &icarus_driver::wait_callback, this, boost::ref(port),
				boost::asio::placeholders::error ) );
