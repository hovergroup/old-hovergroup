...
for i = 1:(N-1)
    ...
    % outputs xHat(i+1)
    if(KFdelay)
        [xHat PKF] = kayakKF(sys,KFparams,zOld,xHat,PKF,u);    
        zOld = z;
        % encode option for mult. time step delay?
    else
        [xHat PKF] = kayakKF(sys,KFparams,z,xHat,PKF,u);
    end
    ...
    [t_out,xcsim]=ode45(@(t,x) kayak_continuous(t,x,Ac,Bc,Bnoise,u,wvec,tspan),tspan,x0ode);
    xcsim=xcsim';
    ...
    if(uDelay)
        [uPlanNext tMPC] = solveKayakMPC(sys,x,MPCparams,uDelay,uPlan(:,1));
    else
        [uPlan tMPC] = solveKayakMPC(sys,x,MPCparams,uDelay,u);
    end
    uPlan
...
for ii = plotStates;
    ...
    set(gca,'Fontsize',16);
    stairs(tvec,CdAll(ii,:)*[XallTrueD [0;0;0;0]],'k');
    hold on
    %stairs(tvec,CdAll(ii,:)*[XallEst [0;0;0;0]],'b');
    plot(tvec,CdAll(ii,:)*[XallEst [0;0;0;0]],'rx')
    plot(tvecC,Cc(ii,:)*[XallTrueC [0 0 0 0]'],'b')
...
subplot(2,1,1)
stairs(tvec,CdAll(4,:)*[XallTrueD [0 0 0 0]'],'k')
hold on
%stairs(tvec,CdAll(4,:)*[XallEst(:,:,1) [0 0 0 0]'],'b')
plot(tvec,CdAll(4,:)*[XallEst [0 0 0 0]'],'rx')
plot(tvecC,Cc(4,:)*[XallTrueC [0 0 0 0]'],'b')
...
stairs(tvec,[Uallmpc(1,1:(end-1)) 0],'k');
title('Rudder Angle')
xlabel('t [sec]');
ylabel('u1 [deg]');
