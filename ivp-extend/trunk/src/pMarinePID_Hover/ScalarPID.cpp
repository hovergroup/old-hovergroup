...
bool ScalarPID::Run(double dfeIn, double dfErrorTime, double &dfOut)
{
  ...
  if(m_nIterations++!=0) {
    ...
    if ( m_update_fraction==0 ) {
    	...
    	m_dfeDiff = m_update_fraction*dfDiffNow +
    			(1-m_update_fraction)*m_dfeDiff;
    }
  }
  else {
...
//-------------------------------------------------------------------
void ScalarPID::SetGains(double dfKp, double dfKd, double dfKi, double update_fraction)
{
  m_dfKp      =   dfKp;
  m_dfKd      =   dfKd;
  m_dfKi      =   dfKi;
