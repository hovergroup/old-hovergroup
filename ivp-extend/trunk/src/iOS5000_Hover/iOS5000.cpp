...
iOS5000_Hover::iOS5000_Hover() : port(io), timeout(io) {
...
void iOS5000_Hover::wait_callback(serial_port& ser_port, const boost::system::error_code& error)
{
	if (error) {
...
void iOS5000_Hover::processLine( double heading, double pitch, double roll, double temp ) {
	heading+=prerotation;
	...
	while (heading_estimate<0) heading_estimate+=360.0;
...
void iOS5000_Hover::serialLoop() {
	while (!stop_requested) {
		...
		port.async_read_some( buffer( &readBuffer[0], 1000 ),
				boost::bind( &iOS5000_Hover::read_handler, this, boost::ref(data_available), boost::ref(timeout),
		...
		timeout.async_wait( boost::bind( &iOS5000_Hover::wait_callback, this, boost::ref(port),
				boost::asio::placeholders::error ) );
