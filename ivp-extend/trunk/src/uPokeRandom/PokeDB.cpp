...
//	cout << hex_data << endl;
	...
	for ( int i=0; i<sub_strings.size(); i++ ) {
		...
		for ( int j=0; j<this_sub.size(); j++ ) {
			switch ( this_sub[j] ) {
			case '0': val += 0; break;
			case '1': val += 1*(16-15*j); break;
			case '2': val += 2*(16-15*j); break;
			case '3': val += 3*(16-15*j); break;
			case '4': val += 4*(16-15*j); break;
			case '5': val += 5*(16-15*j); break;
			case '6': val += 6*(16-15*j); break;
			case '7': val += 7*(16-15*j); break;
			case '8': val += 8*(16-15*j); break;
			case '9': val += 9*(16-15*j); break;
